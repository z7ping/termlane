...
describe('冒烟测试', () => {
  ...
  describe('Mock 命令 — 数据验证', () => {
    test('ssh_connect 返回字符串', async () => {
      const tauri = await import('../utils/tauri.ts');
      try {
        const sessionId = await tauri.invoke('ssh_connect', { host: 'localhost', password: 'test' });
        expect(typeof sessionId).toBe('string');
      } catch {
        // In browser mock, ssh_connect always throws (no real auth)
      }
    });
  ...
  describe('auto-reconnect.js — 类验证', () => {
    test('AutoReconnectManager 类可导入', async () => {
      const { AutoReconnectManager } = await import('../utils/auto-reconnect.ts');
    ...
    test('createReconnectManager 工厂函数存在', async () => {
      const { createReconnectManager } = await import('../utils/auto-reconnect.ts');
      expect(typeof createReconnectManager).toBe('function');
    });

    test('工厂函数返回 AutoReconnectManager 实例', async () => {
      const { createReconnectManager, AutoReconnectManager } = await import('../utils/auto-reconnect.ts');
