/**
 * XTerminal Pro 安全存储模块
 * 提供跨平台的加密存储能力
 *
 * 策略：
 * - Tauri环境：使用系统keyring（@tauri-apps/plugin-secure-storage）
...
/**
 * Tauri环境安全存储实现（系统keyring）
 */
class TauriSecureStorage {
  async set(key, value) {
    try {
      // 动态导入Tauri API（仅在Tauri环境加载）
      const plugin = await import('@tauri-apps/plugin-secure-storage')
      await plugin.setPassword('xterminal-pro', key, JSON.stringify(value))
      return true
    } catch (error) {
      console.error(`Failed to store ${key} in keyring:`, error)
      // 降级到浏览器存储
      const fallback = new BrowserSecureStorage()
  ...
  async get(key) {
    try {
      const plugin = await import('@tauri-apps/plugin-secure-storage')
      const result = await plugin.getPassword('xterminal-pro', key)
      return result ? JSON.parse(result) : null
    } catch (error) {
      console.error(`Failed to retrieve ${key} from keyring:`, error)
      // 尝试从浏览器存储读取
      const fallback = new BrowserSecureStorage()
  ...
  async remove(key) {
    try {
      const plugin = await import('@tauri-apps/plugin-secure-storage')
      await plugin.deletePassword('xterminal-pro', key)
    } catch (error) {
      console.error(`Failed to remove ${key} from keyring:`, error)
    }
  }

  async clear() {
    // Tauri keyring 没有批量删除API，需要逐个删除
    // 这里我们假设应用已知的键
    const knownKeys = [
      'pwd_*',  // 密码
      'bookmarks',
