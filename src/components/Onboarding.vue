      ...
      <div class="onboarding-dialog" role="dialog" aria-modal="true" aria-label="Termlane 首次使用引导" @keydown.esc="skip">
        ...
        <div class="onboarding-content">
          ...
          <section v-else class="step-content">
            ...
            <h2>可以开始了</h2>
            <p>终端和文件是一级工作区；连接延迟、录制、笔记、书签、快捷命令和命令序列在“工具”菜单中。</p>
...
.onboarding-dialog { position: relative; width: min(540px, 100%); max-height: min(620px, 90vh); display: flex; flex-direction: column; overflow: hidden; border: 1px solid var(--border); border-radius: var(--radius-lg); background: var(--bg-elevated); box-shadow: var(--shadow-lg); }
.close-button { position: absolute; top: 10px; right: 10px; z-index: 2; width: 28px; height: 28px; display: inline-flex; align-items: center; justify-content: center; border: 0; border-radius: var(--radius-sm); background: transparent; color: var(--fg-muted); }
...
.step-content { display: flex; flex-direction: column; align-items: center; animation: step-in var(--transition-slow) ease-out; }
...
.feature-grid > div { min-height: 58px; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 6px; border: 1px solid var(--border-subtle); border-radius: var(--radius); background: var(--bg-surface); color: var(--fg-muted); font-size: 10px; }
.notice-card { width: min(430px, 100%); display: flex; align-items: flex-start; gap: 9px; margin-top: 22px; padding: 10px 12px; border: 1px solid color-mix(in srgb, var(--warning) 25%, var(--border)); border-radius: var(--radius); background: color-mix(in srgb, var(--warning) 7%, var(--bg-surface)); color: var(--fg-secondary); font-size: 10px; line-height: 1.55; }
...
.shortcut-grid > div { min-height: 40px; display: flex; align-items: center; gap: 4px; padding: 6px 8px; border: 1px solid var(--border-subtle); border-radius: var(--radius); background: var(--bg-surface); }
.shortcut-grid span { margin-left: auto; color: var(--fg-muted); font-size: 9px; }
kbd { min-width: 23px; padding: 2px 5px; border: 1px solid var(--border); border-radius: 4px; background: var(--bg-base); color: var(--fg-secondary); font-family: var(--font-mono); font-size: 9px; text-align: center; }
...
.secondary-button, .primary-button { height: 30px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; padding: 0 10px; border: 0; border-radius: var(--radius-sm); font-size: 11px; }
.secondary-button { background: var(--bg-hover); color: var(--fg-secondary); }
.primary-button { background: var(--accent); color: white; }
.step-count { color: var(--fg-muted); font-family: var(--font-mono); font-size: 9px; }
...
.onboarding-fade-enter-active, .onboarding-fade-leave-active { transition: opacity var(--transition-fast); }
